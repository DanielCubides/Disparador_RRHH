...
public class EditTriggerServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		try {
			...
			out.printf("Expresi\u00f3n Cron: <input type=text name=exp value=\"%s\" required>", trigger.getCronExpression());
	...
	protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		try {
			...
			StdSchedulerFactory factory = (StdSchedulerFactory) ctx.getAttribute(QuartzListener.QUARTZ_FACTORY_KEY);
			Scheduler scheduler = factory.getScheduler();
			Trigger trigger = TriggerBuilder.newTrigger().withIdentity("simple").withSchedule(CronScheduleBuilder.cronSchedule(cronExpression)).startNow().build();
			Date date = scheduler.rescheduleJob(new TriggerKey("simple"), trigger);
			out.printf(new Locale("es", "CO"), "Trabajo re-calendarizado<br>Próxima ejecución:<br>%tc", date);
		} catch (Exception e) {
			out.print("<pre>");
			e.printStackTrace(out);
			out.print("</pre>");
